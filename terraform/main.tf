provider "aws" {
  region = var.region
  access_key = "AKIATMQVTJSUPTAU2BCY"
  secret_key = "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"
}
...
resource "aws_s3_bucket" "image_bucket" {
  bucket = "yalantis-image-bucket-${var.environment}"
}
#
# SNS Topic
...
resource "aws_lambda_function" "notify_upload" {
  filename = "../email-lambda/target/email-lambda-0.0.1-SNAPSHOT.jar"
